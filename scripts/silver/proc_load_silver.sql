...
(Extract, transform and load) process and populates the tables in 
the silver layer.
Actions performed in this script include:
	- Truncating the tables before inserting them to avoid 
	  duplicates.
	- Inserts transformed and cleaned data into the tables.

Since this code itself does not deliver any output the query:
'EXEC silver.load_silver;' must be used.
*/
	...
	BEGIN TRY 
		...
		SELECT 
			CASE WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid, 4, LEN(cid))
				 ELSE cid 
			END AS cid, 
			CASE WHEN bdate < '1875-01-01' OR bdate > GETDATE() THEN NULL
				 ELSE bdate
			END AS bdate,
			CASE WHEN LOWER(gen) in ('F', 'Female') THEN 'Female'
		...
		PRINT '>> entire silver layer loading duration ' + CAST(DATEDIFF(second, @batch_start_time, @batch_end_time) AS NVARCHAR) + ' seconds';
		PRINT '------------------------------------------------------------------------------------------------------------------------------';
